...
def query_which_cloud() -> str:
    if (
        subprocess.call(
            'curl -f --connection-timeout 10 --noproxy "*" http://169.254.169.254/1.0/meta-data/instance-id'.split(),
        ...
        subprocess.call(
            'curl -f --connection-timeout 10 -H Metadata:true --noproxy "*" "http://169.254.169.254/metadata/instance?api-version=2021-02-01"'.split(),
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        == 0
        ...
        subprocess.call(
            'curl -f --connection-timeout 10 -noproxy "*" http://metadata.google.internal/computeMetadata/v1/instance/hostname'.split(),
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        == 0
