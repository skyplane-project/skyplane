...
RUN --mount=type=cache,target=/root/.cache/pip pip3 install --compile -r /tmp/requirements-gateway.txt && rm -r /tmp/requirements-gateway.txt

WORKDIR /pkg
COPY . .
RUN --mount=type=cache,target=/root/.cache/pip pip3 install -e .
