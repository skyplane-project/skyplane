...
class TransferProgressTracker(Thread):
        ...
        for job_uuid, job in self.jobs.items():
            ...
            try:
                ...
                for chunk in chunk_stream:
                    ...
                    print("DISPATCHED", chunk.chunk_id, job_uuid)
            ...
            print("AFTER DISPATCH", len(list(self.job_chunk_requests.keys())), job_uuid, self.job_chunk_requests)

            self.hooks.on_dispatch_end()

            print("MAPPING", self._chunk_to_job_map)
        ...
        while any([len(self.job_pending_chunk_ids[job_uuid][region_tag]) > 0 for job_uuid in self.job_pending_chunk_ids]):
            ...
            for job_uuid, job in self.jobs.items():
                try:
                    ...
                    print("chunk_to_job", self._chunk_to_job_map)
                    print("chunk req", self.job_chunk_requests)
                    raise e
                new_chunk_ids = (
                ...
                for id in new_chunk_ids:
                    assert job_uuid in self.job_chunk_requests and id in self.job_chunk_requests[job_uuid], f"Missing chunk id {id} for job {job_uuid}: {self.job_chunk_requests}"
                for id in new_chunk_ids:
    ...
    def _chunk_to_job_map(self):
        for job in self.job_chunk_requests.keys(): 
            print(f"all chunks {job}", list(self.job_chunk_requests[job].keys()))
