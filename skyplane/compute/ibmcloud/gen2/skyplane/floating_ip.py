...
class FloatingIpConfig(ConfigBuilder):
    def run(self) -> Dict[str, Any]:
        ...
        free_floating_ips = [
            ip for ip in floating_ips if not ip.get("target") and self.base_config["provider"]["zone_name"] == ip["zone"]["name"]
        ]

        if free_floating_ips:
            ALLOCATE_NEW_FLOATING_IP = "Allocate new floating ip"
            head_ip_obj = get_option_from_list(
                "Choose head ip", free_floating_ips, choice_key="address", do_nothing=ALLOCATE_NEW_FLOATING_IP
            )
