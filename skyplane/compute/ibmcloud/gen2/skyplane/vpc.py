...
def validate_security_group(ibm_vpc_client, sec_group_id):
    ...
    if errors:
        ...
        if not errors:
            ...
            print(
                f"Something failed during security group rules update/create, please update the required rules manually using ibmcli or web ui and try again"
            )
            exit(1)
    else:
        return
...
def add_rules_to_security_group(ibm_vpc_client, sg_id, missing_rules):
    ...
    for missing_rule in missing_rules.keys():
        q = [inquirer.List("answer", message=add_rule_msgs[missing_rule], choices=["yes", "no"], default="yes")]
