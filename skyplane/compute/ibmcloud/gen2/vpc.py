...
from skyplane.compute.ibmcloud.gen2.utils import (
    find_default,
    find_name_id,
...
class VPCConfig(ConfigBuilder):
    ...
    def run(self) -> List:
        ...
        subnet_objects = [
            s_obj for s_obj in all_subnet_objects if s_obj["zone"]["name"] == zone_obj["name"] and s_obj["vpc"]["id"] == vpc_obj["id"]
        ]
    ...
    def _create_vpc(self, ibm_vpc_client, resource_group, vpc_default_name, auto=False):
        ...
        q = [
            ...
            inquirer.List(
                "answer",
                message="Create new VPC and configure required rules in default security group",
                choices=["yes", "no"],
                default="yes",
            ),
        ]

        answers = {"answer": "yes", "name": vpc_default_name}
        ...
        if answers["answer"] == "yes":  # create a new VPC
            ...
            @spinner
            def _create():
                return ibm_vpc_client.create_vpc(
                    address_prefix_management="auto", classic_access=False, name=answers["name"], resource_group=resource_group
                ).get_result()
    ...
    def create_public_gateway(self, vpc_obj, zone_obj, resource_group, subnet_name):
        ...
        gateway_data = self.ibm_vpc_client.create_public_gateway(**gateway_prototype).get_result()
    ...
    def _select_zone(self, vpc_id, region, auto=False):
        ...
        try:
            default = find_default(self.defaults, zones_objects, name="zone_name")
    ...
    def _select_vpc(self, node_config, region):
        ...
        while True:
            ...
            # User chose an existing VPC
                ...
                subnet_objects = [
                    s_obj
                    for s_obj in all_subnet_objects
                    if s_obj["zone"]["name"] == zone_obj["name"] and s_obj["vpc"]["id"] == vpc_obj["id"]
                ]
    ...
    def verify(self, base_config):
        ...
        if self.defaults["vpc_id"]:
            ...
            if vpc_objects["total_count"] > 0:
                ...
                vpc_obj = self.ibm_vpc_client.create_vpc(
                    address_prefix_management="auto", classic_access=False, name="skyplane-default-vpc", resource_group=resource_group
                ).get_result()
    ...
    def create_default(self):
        ...
        if vpc_obj:
            ...
            vpc_obj = self._create_vpc(self.ibm_vpc_client, resource_group, self.vpc_name, auto=True)
            if not vpc_obj:
