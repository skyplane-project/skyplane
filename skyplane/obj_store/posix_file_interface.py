...
class POSIXInterface(ObjectStoreInterface):
    ...
    def list_objects(self, prefix="") -> Iterator[POSIXFile]:
        ...
        for root, dirs, files in os.walk(self.path):
            for file in files:
                ...
                yield POSIXFile(provider="posix", bucket=self.path, key=full_path, size=os.path.getsize(full_path), last_modified=os.path.getmtime(full_path))
    ...
    def download_object(self, src_object_name, dst_file_path, offset_bytes=None, size_bytes=None, write_at_offset=False, generate_md5: bool = False):
    ...
    def upload_object(self, src_file_path, dst_object_name, part_number=None, upload_id=None, check_md5: Optional[bytes] = None, mime_type: Optional[str] = None):
    ...
    def get_object_metadata(self, obj_name: str) -:
        ...
        if not self.exists(obj_name):
            raise NoSuchObjectException(obj_name)
        return {"size": os.path.getsize(obj_name), "last_modified": os.path.getmtime(obj_name), "mime_type": self.get_obj_mime_type(obj_name)}