...
class BCCopyJob(BCTransferJob):
    ...
    def gen_transfer_pairs(self, chunker: Optional[BCChunker] = None) -> Generator[Tuple[ObjectStoreObject, ObjectStoreObject], None, None]:
        ...
        if self.transfer_config.gen_random_data:
            if chunker is None:  # used for external access to transfer pair list
                chunker = BCChunker(transfer_config=self.transfer_config)

            yield from chunker.transfer_pair_random_generator()

        else:
            if chunker is None:  # used for external access to transfer pair list
                chunker = BCChunker(self.src_iface, list(self.dst_ifaces.values()), self.transfer_config)
    ...
    def broadcast_dispatch(
        ...
        if not transfer_config.gen_random_data:
            ...
            chunker = BCChunker(transfer_config=self.transfer_config, num_partitions=dataplane.topology.num_partitions)
