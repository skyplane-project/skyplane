...
class ReplicatorClient:
    ...
    def initiate_multipart_requests(self, job, multipart_pairs): 
    ...
    def run_replication_plan(
        ...
        # TODO: support broadcast replication job
        ...
        with Progress(SpinnerColumn(), TextColumn("Preparing replication plan{task.description}"), transient=True) as progress:
                ...
                if job.random_chunk_size_mb:
                    chunks.append(
                        Chunk(
                            ...
                            chunk_id=idx,
                            partition_id=idx % self.topology.num_blocks,  # partition id of chunk 
                            file_offset_bytes=0,
            ...
            if len(multipart_pairs) > 0: 
                self.initiate_multipart_requests(job, multipart_pairs)
                    ...
                    for chunk in batch:
                        chunk_requests_sharded[batch_idx].append(
                            ChunkRequest(
                                ...
                                src_object_store_bucket=None, #job.source_bucket,
                                dst_object_store_bucket=None #job.dest_bucket,
                            )
                        )
