...
df = df[df["problem_instance_limit"] == instance_limit].reset_index(drop=True)
...
with plt.style.context(style):
    ...
    for i, src in enumerate(label_map.keys()):
        for j, dest in enumerate(label_map.keys()):
            ax = axs[i, j]
            ax.set_title(f"{label_map[src]} to {label_map[dest]}", fontsize=12)
            # show histogram
            df_sub = df[(df["src_provider"] == src) & (df["dst_provider"] == dest)]
            df_sub = df_sub[(df_sub["is_feasible"]) & (df_sub["throughput_speedup"] > 1.0)]

            # stack throughput_achieved_gbits and baseline_throughput_achieved_gbits with a label column saying "Overcloud" and "Overcloud, no Overlay"
            # rows = []
            # for row in df_sub.itertuples():
            #     rows.append({"Throughput (Gbits)": row.throughput_achieved_gbits, "label": "Overcloud + overlay"})
            #     rows.append({"Throughput (Gbits)": row.baseline_throughput_achieved_gbits, "label": "Overcloud, no overlay"})
    ...
    # put two row legend below plot
    axs[0][1].legend(loc="upper center", bbox_to_anchor=(0.5, 1.75), ncol=2, fontsize=14, frameon=False)
...
#         for dst in label_map.keys():
#             df_sample = df[['throughput_speedup', 'cost_increase', 'src_provider', 'dst_provider']]
#             df_sample = df_sample[(df_sample['src_provider'] == src) & (df_sample['dst_provider'] == dst)]
#             df_sample = df_sample[df_sample['throughput_speedup'] > 1.05]
#             sns_fig = sns.jointplot(data=df_sample, x="cost_increase", y="throughput_speedup", kind="hist")
#             sns_fig.ax_joint.set_title(f"{label_map[src]} to {label_map[dst]}")
#             # log-log
#             # sns_fig.ax_joint.set_xscale("log")
#             # sns_fig.ax_joint.set_yscale("log")
#             st.pyplot(sns_fig)
